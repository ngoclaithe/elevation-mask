...
Default: Florence + SAM on, VL critic **off** (CPU 7B/3B is minutes per loop). Geometry critic always runs.
...
```bash
python -m app.cli samples/image_1.jpg --no-florence --no-sam
```
